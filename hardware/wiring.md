...
The code uses `rpi-hardware-pwm` which talks to `/sys/class/pwm/pwmchip2`. That sysfs interface is only exposed once you enable the PWM device-tree overlay. Edit `/boot/firmware/config.txt` and add (or merge into) the line:
